...
df_vendas = pd.read_csv(
    pasta_datasets / "vendas.csv", decimal=",", sep=";", index_col=0
)
...
lista_vendedores = df_filiais.loc[
    df_filiais["cidade/estado"] == filial_selecionada, "vendedores"
].iloc[0]
...
genero_selecionado = st.sidebar.selectbox(
    "Genêro do cliente:", ["masculino", "feminino"]
)

forma_pag_selecionado = st.sidebar.selectbox(
    "Forma de pagamento", ["credito", "pix", "boleto"]
)
