...
colunas_filtradas = [c for c in COLUNAS_ANALISE if c not in indice_dinamica]
coluna_dinamica = st.sidebar.multiselect("Selecione os colunas:", colunas_filtradas)
...
if len(indice_dinamica) > 0 and len(coluna_dinamica) > 0:
    ...
    vendas_dinamica = pd.pivot_table(
        df_vendas,
